    ...
    <div class="ui-posts" itemscope itemtype="http://schema.org/Blog">
      ...
      >
        ...
        <header class="ui-post-title" itemprop="name headline">
          <!-- 主要修改了这里，在有 feishu 这个属性的时候，跳转到飞书 -->
          <!-- 原本的源码在 https://github.com/vuepress/vuepress-theme-blog/blob/master/global-components/BaseListLayout.vue -->
          <NavLink
            v-if="page.frontmatter.feishu"
            :link="page.frontmatter.feishu"
            >{{ page.title }}
          </NavLink>
          <NavLink v-else :link="page.path">{{ page.title }} </NavLink>
        </header>
